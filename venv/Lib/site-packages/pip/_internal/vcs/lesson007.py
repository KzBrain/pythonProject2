...
class Matrix():
    def __init__(self,digit1,digit2,digit3,digit4):
        self.d1 = digit1
        self.d2 = digit2
        self.d3 = digit3
        self.d4 = digit4
    def __str__(self):
        return f'{self.d1} " " {self.d2}  " " {self.d3}  " " {self.d4} '

    def __add__(self, other):
        return self.d1 + other.d1 ,self.d2 + other.d2,self.d3 + other.d3,self.d4 + other.d4
one = Matrix(2,3,4,5)
two = Matrix(3,4,5,6)
print(one + two)
...
class Tissue:
    ...
    def square_full(self):
        return f'{( self.width /6.5 + 0.5) + (self.height * 2 + 0.3)}'
...
class Cell:
    ...
    def __sub__(self, other):
        if self.digit - other.digit > 0 :
           return self.digit - other.digit
        else: 'Fault'

    def __truediv__(self, other):
        return round(self.digit / other.digit)

    def make_order(self, cells_in_row):
        row = ''
        for i in range(int(self.digit / cells_in_row)):
            row += f'{"*" * cells_in_row} \\n'
        row += f'{"*" * (self.digit % cells_in_row)}'
        return row
